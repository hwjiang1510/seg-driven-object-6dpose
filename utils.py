...
def vertices_reprojection_FPHA(vertices, rt, k, extrinsic):
    '''
    ...
    :return:
    '''
    vertices = vertices * 1000.0 # from meter to millimeter
    #p = np.matmul(extrinsic[:3,0:3], vertices.T) + extrinsic[:3,3].reshape(-1,1)
    p = np.matmul(k, np.matmul(rt[:3, 0:3], vertices.T) + rt[:3, 3].reshape(-1, 1))
    p[0] = p[0] / (p[2] + 1e-5)
    p[1] = p[1] / (p[2] + 1e-5)
    return p[:2].T
...
def do_detect(model, rawimg, intrinsics, bestCnt, conf_thresh, use_gpu=False):
    ...
    height, width, _ = rawimg.shape# 480,640

    # scale
    img = cv2.resize(rawimg, (model.width, model.height)) #608,608
    ...
    out_preds = model(img)
    #out1 = self.segLayer(outlayers[0]), out_preds = [cls_confs, cls_ids]
    #out2 = self.regLayer(outlayers[1]), out_preds = [px, py, conf, keypoints]
    #out_preds = [out1, out2]
    #print(out_preds[1][3].shape)

    t3 = time.time()

    predPose = fuse_test(out_preds, width, height, intrinsics, conf_thresh, 0, bestCnt)
...
def fuse_test(output, width, height, intrinsics, conf_thresh, batchIdx, bestCnt):
    layerCnt = len(output)
    assert(layerCnt == 2)

    '''
    outlayers = core_model(img)
    #out1 = self.segLayer(outlayers[0]), out_preds = [cls_confs, cls_ids]
    #out2 = self.regLayer(outlayers[1]), out_preds = [px, py, conf, keypoints]
    #out_preds = [out1, out2]
    '''
    cls_confs = output[0][0][batchIdx]#cls_confs,(76,76)
    cls_ids = output[0][1][batchIdx]#(76,76)
    predx = output[1][0][batchIdx] #(76,76,8)
    predy = output[1][1][batchIdx]
    det_confs = output[1][2][batchIdx] #(76,76,8)
    keypoints = output[1][3] #(8,8,3)
    ...
    mdConf = det_confs.mean(axis=2) # average 2D confidences,(76,76)
    for cidx in range(nC):
        # skip background
        if cidx == 0:
            continue
        foremask = (cls_ids == cidx)#(76,76)
        cidx -= 1
...
def get_bbox8_3d(vertex):
    ...
    for i in range(cls):
        ...
        pts = np.stack((p_blb,p_brb,p_blf,p_brf,p_tlb,p_trb,p_tlf,p_trf))
        bbox.append(pts)
    bbox = np.array(bbox)
    return bbox
...
def visualize_bbox(predPose, image, bbox, intrinsics):
    height, width, _ = image.shape
    for p in predPose:
        outid, rt, conf, puv, pxyz, opoint, clsid, partid, cx, cy, layerId = p
        points = vertices_reprojection(bbox[outid][:], rt, intrinsics)
        points = points.astype(int)

        center_bottem = (points[0] + points[3]) // 2
        center_top = (points[4] + points[7]) // 2
        center = (center_bottem + center_top) // 2

        center_left = (points[0] + points[6]) // 2
        center_behind = (points[4] + points[1]) // 2

        #bottem box
        image = cv2.line(image, tuple(points[0]), tuple(points[1]), get_class_colors(outid), 2, 8)
        image = cv2.line(image, tuple(points[0]), tuple(points[2]), get_class_colors(outid), 2, 8)
        image = cv2.line(image, tuple(points[1]), tuple(points[3]), get_class_colors(outid), 2, 8)
        image = cv2.line(image, tuple(points[2]), tuple(points[3]), get_class_colors(outid), 2, 8)
        #top box
        image = cv2.line(image, tuple(points[4]), tuple(points[5]), get_class_colors(outid), 2, 8)
        image = cv2.line(image, tuple(points[4]), tuple(points[6]), get_class_colors(outid), 2, 8)
        image = cv2.line(image, tuple(points[5]), tuple(points[7]), get_class_colors(outid), 2, 8)
        image = cv2.line(image, tuple(points[6]), tuple(points[7]), get_class_colors(outid), 2, 8)
        #vertical box
        image = cv2.line(image, tuple(points[0]), tuple(points[4]), get_class_colors(outid), 2, 8)
        image = cv2.line(image, tuple(points[1]), tuple(points[5]), get_class_colors(outid), 2, 8)
        image = cv2.line(image, tuple(points[2]), tuple(points[6]), get_class_colors(outid), 2, 8)
        image = cv2.line(image, tuple(points[3]), tuple(points[7]), get_class_colors(outid), 2, 8)
        # directions
        image = cv2.line(image, tuple(center), tuple((center_left+center)//2), (0,0,255), 2, 4) #R
        image = cv2.line(image, tuple(center), tuple((center_top+center)//2), (255,0,0), 2, 4) #B
        image = cv2.line(image, tuple(center), tuple((center_behind+center)//2), (0,255,0), 2, 4) #G

    return image

def visualize_predictions(predPose, image, vertex, intrinsics):
    # vertex shape: (8,10000,3), the original mesh
    height, width, _ = image.shape
